...
if(INJECT){
  ...
  /* Сьома (S19): чипи сходинок перестають вчитися з відповіді. Ряд на
     екрані цілий, номери на місці, перемикання працює — просто назва
     моделі не з'являється ніколи, і оператор вмикає сходинку наосліп.
     Ціль обрана так, щоб падало РІВНО одне твердження: підпис вивченого
     чипа. Зачепиш замість цього самі підписи — почервоніє ще й «до першої
     відповіді номер, не модель», і два твердження стануть дзеркалом
     одного дефекту (12.12-ї). */
  html=html.replace("  learn(rung, model){\n    if(!model) return;",
                    "  learn(rung, model){\n    return;");
}
...
(async()=>{
  ...
  /* Було два (пояснення про ключ, технічні дані); 6а-2 додав третій —
     «Перевірка даних»; S19 додав четвертий — пояснення піна сходинки.
     Число тримається навмисно: воно ловить акордеон, що зʼявився повз
     рішення. Піднімати його можна тільки разом із записом у самері.
     Порядковий номер у підписі не називаємо: акордеон вставляється в
     середину екрана, і «третій» перестає бути правдою від однієї вставки,
     хоч перевірка й шукає по імені. */
  const accs=[...d.querySelectorAll('details.acc')];
  T('акордеонів чотири', accs.length===4);
  T('акордеон «Перевірка даних» на місці',
    accs.some(a=>a.querySelector('summary')&&a.querySelector('summary').textContent==='Перевірка даних'));
  T('акордеон піна пояснює, що драбина сама нижче не сходить',
    accs.some(a=>/тільки.*квота вичерпана|не викликаються ніколи/.test(a.textContent)));
  ...
  /* ── ПІН СХОДИНКИ (S19) ────────────────────────────────────────────
     Ряд чипів читаємо з DOM, а не з джерела: підпис чипа — це і є те, що
     оператор побачить, коли вирішуватиме, чи вмикати сходинку. */
  console.log('\n— пін сходинки —');
  const rungRow=[...d.querySelectorAll('.tabs')].pop();
  const rungBtns=rungRow?[...rungRow.querySelectorAll('button')]:[];
  T('ряд сходинок: авто + чотири сходинки', rungBtns.length===5);
  T('перший чип — «авто», і він натиснутий на чистому браузері',
    !!rungBtns[0] && rungBtns[0].textContent==='авто' &&
    rungBtns[0].getAttribute('aria-pressed')==='true');
  /* Головне твердження ходу: назв моделей у застосунку НЕ оголошено.
     Доки жодної відповіді не було, чип уміє назвати лише номер. */
  T('до першої відповіді чипи називають номер, не модель',
    rungBtns.slice(1).every(b=>/^сходинка \d$/.test(b.textContent)));
  /* Друга половина того самого: назва приходить із заголовка відповіді.
     Кличемо PIN.learn напряму — саме її кличе callModel, прочитавши
     x-ae-model. Так перевіряється шлях, а не факт існування функції. */
  w.PIN.learn(2,'постачальник/модель-стенду');
  w.render();
  const learned=[...[...d.querySelectorAll('.tabs')].pop().querySelectorAll('button')];
  T('вивчена з відповіді назва стає підписом чипа',
    learned[3] && learned[3].textContent==='модель-стенду');
  T('решта чипів лишились номерами — вчиться саме той, хто відповів',
    learned[2] && /^сходинка 1$/.test(learned[2].textContent));
  T('[джерело] заголовок піна йде тільки через воркер, не з ключем',
    /if\(!key && PIN\.get\(\)!==''\) headers\['x-ae-pin'\]/.test(src));
