/* AE-Simulator · валідатор цілісності даних
   живе доки: catalog.json + scenarios.json лишаються джерелом даних тренажера
   запуск: node ae_validate.js   (читає data/catalog.json і data/scenarios.json)           */
const fs=require('fs');
const _c=JSON.parse(fs.readFileSync(process.argv[2]||'data/catalog.json','utf8'));
const CAT=_c.categories?Object.fromEntries(Object.entries(_c.categories).map(([k,v])=>[k,v.items])):_c;
const _s=JSON.parse(fs.readFileSync(process.argv[3]||'data/scenarios.json','utf8'));
const SCEN=_s.scenarios||_s;
let ok=0,warn=0,err=0;
const E=m=>{console.log('  ✗ '+m);err++};
