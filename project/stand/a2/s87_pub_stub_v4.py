# s87_pub_stub_v4.py · детектор D2 крок 5 + 5в «тиша для Олі» — публікація з острова КНОПКОЮ, на заглушках (корінь під ?a2v2)
# живе доки: D2 крок 8 (SPEC, П116) — далі в archive/stand. v2 (S86) = пункти 1–22; v3 додає 23а–23г (5в предмет 1, S87):
# v4 (5в предмет 2, S87): пункт 20 розрізано — редактор читає main, а не копію Pages:
#   20а сторінка відстала, чернетки нема → сторінка бере main, смуги нема, публікація жива ·
#   20б main змінився в ТОМУ Ж записі, що в чернетці → зупинка, як була · 20в чернетка в ІНШОМУ записі → бере main, чернетка ціла.
#   Негативний контроль 20а·20в — X108 (fd9b3053): ✗; 20б ✓ на обох.
#   23а звірка на вході тиха (смуга схована, кнопка вимкнена) · 23б фокус і введене в назві переживають кінець звірки ·
#   23в звірка довша за 1 с — смуга з'являється · 23г без ?a2v2 старий редактор, як і був, показує вердикт звірки.
# Затримка заглушки — утримання route (HOLD) і fulfill з головного потоку: sleep у обробнику блокує sync-API.
# Негативний контроль — X107 (d7c81530): 23а·23б ✗ (смуга на вході, фокус злітає); 23в — сторож (смуга мусить бути) ✓ на обох. X108: 31 ✓.
...
# Очікувано 1–22 на X107 і далі: усі ✓. Негативний контроль — X106 (4254b656): кнопка вимкнена (Р1) → ✗ з першого кроку запису.
# Запуск: python3 s87_pub_stub_v4.py <корінь AE з білдом index.html>
...
CAP=[]; HOLD=[False]; PEND=[]; REPO={'data/scenarios.json':[(AE/'data/scenarios.json').read_bytes(),'stub-0'], 'data/catalog.json':[(AE/'data/catalog.json').read_bytes(),'cat-0']}
def gh(route):
  if HOLD[0]: PEND.append(route); return
...
with sync_playwright() as p:
  ...
  pg2.wait_for_timeout(150); t0=J2("cur(st.sel).title"); J2("set('title', cur(st.sel).title+' x')"); pg2.wait_for_timeout(150); sy=J2(SYNC)
  ok('20а сторінка відстала, чернетки нема → бере main: смуги нема, публікація жива', sy and sy['h'] and sy['pub'] and 'чужа правка' in t0 and J2("S.epub.state")=='ok', t0+' '+str(sy))
  def entry_hold(pre):
    pg2.goto(U); pg2.wait_for_function("typeof S!=='undefined' && S.SCEN && S.SCEN.length>0 && S.P", timeout=15000)
    HOLD[0]=True; J2("S.screen='ascen'; render()"); pg2.wait_for_selector('#newSc', timeout=3000); pg2.wait_for_timeout(200)
    J2(pre); pg2.wait_for_timeout(100); HOLD[0]=False
    for r in PEND: gh(r)
    PEND.clear()
    try: pg2.wait_for_function("S.epub && S.epub.state!=='busy'", timeout=5000)
    except Exception: pass
    pg2.wait_for_timeout(200); return J2(SYNC)
  sid=str(alt['scenarios'][0]['id'])
  sy=entry_hold("st.sel=%r; drawEditor(); set('title', cur(%r).title+' b')" % (sid,sid))
  ok('20б main змінився в тому ж записі, що в чернетці → зупинка + «Звірити», публікація вимкнена', sy and not sy['h'] and sy['tone']=='bad' and sy['again'] and not sy['pub'], str(sy))
  oid=str(alt['scenarios'][1]['id'])
  sy=entry_hold("st.sel=%r; drawEditor(); set('title', cur(%r).title+' c')" % (oid,oid))
  v=J2("({t0:S.SCEN[0].title, d:cur(%r).title, k:Object.keys(S.edraft)})" % (oid,))
  ok('20в чернетка в іншому записі → бере main, чернетка ціла, публікація жива', sy and sy['h'] and sy['pub'] and 'чужа правка' in v['t0'] and v['d'].endswith(' c') and oid in v['k'], str(v)+' '+str(sy))
  ...
  # ── 5в предмет 1: тиха звірка на вході (сторінка 3, звірка утримується) ──
  REPO['data/scenarios.json']=[(AE/'data/scenarios.json').read_bytes(),'stub-0']
  pg3=b.new_page(viewport={'width':1920,'height':1080}); pg3.on('pageerror',lambda e:E.append(str(e)))
  pg3.add_init_script("try{localStorage.setItem('ae_curator_until', String(Date.now()+36e5))}catch(_){}")
  pg3.route(re.compile(r'^https://api\.github\.com/'), gh); pg3.route(re.compile(r'^https://ae-edit\.konstandre\.workers\.dev'), wk)
  pg3.goto(U); pg3.wait_for_function("typeof S!=='undefined' && S.SCEN && S.SCEN.length>0 && S.P", timeout=15000)
  J3=pg3.evaluate; HOLD[0]=True
  J3("S.screen='ascen'; render()"); pg3.wait_for_selector('#newSc', timeout=3000); pg3.wait_for_timeout(300); sy=J3(SYNC)
  ok('23а звірка на вході тиха: смуга схована, кнопка вимкнена', J3("S.epub&&S.epub.state")=='busy' and sy['h'] and not sy['pub'], str(sy))
  pg3.click('#f-title'); pg3.keyboard.press('End'); pg3.keyboard.type('Q'); pg3.wait_for_timeout(900); sy=J3(SYNC)
  ok('23в звірка довша за 1 с — смуга з\'являється', J3("S.epub.state")=='busy' and not sy['h'] and sy['tone']=='busy', str(sy))
  HOLD[0]=False
  for r in PEND: gh(r)
  PEND.clear()
  try: pg3.wait_for_function("S.epub && S.epub.state!=='busy'", timeout=5000)
  except Exception: pass
  pg3.wait_for_timeout(200)
  f=J3("({a:document.activeElement&&document.activeElement.id, v:document.querySelector('#f-title').value, s:S.epub.state})")
  ok('23б фокус і введене в назві переживають кінець звірки', f['a']=='f-title' and f['v'].endswith('Q') and f['s']=='ok', str(f))
  pg4=b.new_page(viewport={'width':1920,'height':1080}); pg4.on('pageerror',lambda e:E.append(str(e)))
  pg4.add_init_script("try{localStorage.setItem('ae_curator_until', String(Date.now()+36e5))}catch(_){}")
  pg4.route(re.compile(r'^https://api\.github\.com/'), gh); pg4.route(re.compile(r'^https://ae-edit\.konstandre\.workers\.dev'), wk)
  pg4.goto(U.replace('?a2v2','')); pg4.wait_for_function("typeof S!=='undefined' && S.SCEN && S.SCEN.length>0 && S.P", timeout=15000)
  pg4.evaluate("S.screen='ascen'; render()")
  try: pg4.wait_for_function("S.epub && S.epub.state==='ok' && [...document.querySelectorAll('.publine')].some(p=>/Звірено/.test(p.textContent))", timeout=5000); g=True
  except Exception: g=False
  ok('23г без ?a2v2 старий редактор показує вердикт звірки (render, як було)', g, pg4.evaluate("S.epub&&S.epub.state"))
