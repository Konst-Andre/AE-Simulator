# s85_olya_walk_v5_root.py · прохід «як Оля» (П115/П117) — v4 цілком, перенесений з мокапа на КОРІНЬ продукту під ?a2v2 (D2 крок 4)
# живе доки: D2 крок 8 (SPEC, П116) — далі в archive/stand; детектор кроків 5–7 бере його ж.
...
# Очікувано на кроці 4: 59 ✓ · 7 ✗ — 24–29 (дефолт групи, власник — крок 6, В-142) і 30 (умова Н10 «публікація не блокується»:
# під Р1 публікація вимкнена, власник — крок 5). Інший ✗ — дефект.
# Негативний контроль: --no-flag → острова немає → вихід 2 одразу.
# Запуск: python3 s85_olya_walk_v5_root.py <корінь AE з білдом index.html> [--no-flag]
import sys, json, pathlib, threading, http.server, socketserver, functools
