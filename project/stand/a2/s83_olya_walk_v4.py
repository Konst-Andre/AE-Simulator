# s83_olya_walk_v4.py · сценарний прохід «як Оля» (П115/П117) по D1 v4.5 (база — v3 цілком + кроки 24–38) · живе доки: A2 v2 не перенесено в продукт (D2)
# База — s77_olya_walk_v1.py цілком; змінено 2b (перенос замість обрізання), 6a (ЗФ — з підказкою, В-107), 8 (✎ ліворуч, ≤40 px).
# Запуск: python3 s83_olya_walk_v4.py <стенд.html> <корінь AE>
...
with sync_playwright() as p:
  ...
  for w,h in ((1920,1080),(1536,864),(1280,800)):
    ...
    J("toast('Перевірка тоста')"); pg.wait_for_timeout(250); J("window.scrollTo(0,document.documentElement.scrollHeight)"); pg.wait_for_timeout(200)
    ...
    J("window.scrollTo(0,0)"); pg.wait_for_timeout(2600)
