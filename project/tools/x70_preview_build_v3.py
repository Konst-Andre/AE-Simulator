...
# -*- coding: utf-8 -*-
"""
x70_preview_build_v3.py — AE-Simulator · АВТОНОМНЕ ПРЕВ'Ю

v3 (S70): вшиває ще й <script src="tools/ae_rules.js">. v2 перехоплював лише fetch,
а правила вантажаться тегом — з file:// AE_RULES не існувало, і A7 «Система»
та редактор у прев'ю показували «правила не завантажились». Решта — як v2.
...
⚠ Прев'ю — НЕ білд для передачі. Продукт — корінь репо index.html (В-139, S82).
  Прев'ю живе у sandbox (П120).
"""
import base64, hashlib, json, pathlib, sys

SRC  = pathlib.Path(sys.argv[1]) if len(sys.argv)>1 else pathlib.Path('AE_WORK_index_X27P5_v1.html')
DST  = pathlib.Path(sys.argv[2]) if len(sys.argv)>2 else pathlib.Path('AE_WORK_PREVIEW_X27P5_v1.html')
...
/* ── АВТОНОМНЕ ПРЕВ'Ю ────────────────────────────────────────────────────
   ...
   Це прев'ю для перегляду верстки, не білд для передачі (В-139: продукт — корінь index.html). */
(function(){
  var DATA = __BLOB__;
  ...
  /* X71 · doorPreview() пізнає прокладку рівно за цим прапорцем
     (продукт :989). Без нього двері в прев'ю йдуть у мережу, і
     кураторський режим у прев'ю не відкривається — тобто хід
     неможливо пред'явити (П83). Продукт не чіпається (П81). */
  window.fetch.__aeShim = true;
})();
</script>
"""
    ...
    if t.count(RULES_TAG) != 1:
        print(f'  ✗ тег правил: {t.count(RULES_TAG)} входжень, очікував 1'); return 4
    rules = (ROOT / 'tools/ae_rules.js').read_text(encoding='utf-8')
    if '</script' in rules.lower():
        print('  ✗ ae_rules.js містить </script — вшивати не можна'); return 5
    t = t.replace(RULES_TAG, '<script id="AE-PREVIEW-RULES">\n' + rules + '\n</script>', 1)
    print(f'  вшито tools/ae_rules.js · {len(rules.encode())} б')
