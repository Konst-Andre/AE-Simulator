...
const files={
  'config.json':'config.json','data/catalog.json':'data/catalog.json','data/scenarios.json':'data/scenarios.json'};
...
const dom=new JSDOM(html,{runScripts:'dangerously',url:'https://x.test/',
  beforeParse(w){
    w.fetch=(u)=>Promise.resolve({json:()=>Promise.resolve(
      JSON.parse(fs.readFileSync(path.join(BASE,files[u]||u),'utf8')))});
    w.scrollTo=()=>{};
...
(async()=>{
  ...
  T('розбір зʼявився', S.feedback.length>40);
